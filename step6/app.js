...
function setupTodoForm() {
  ...
  todoTitleInput.addEventListener("input", () =>
    clearError(todoTitleInput, titleError)
  );
  todoAssigneeInput.addEventListener("input", () =>
    clearError(todoAssigneeInput, assigneeError)
  );
  todoDeadlineInput.addEventListener("input", () =>
    clearError(todoDeadlineInput, deadlineError)
  );
...
function validateForm() {
  ...
  if (!deadline) {
    showError(
      todoDeadlineInput,
      deadlineError,
      "Bitte wähle ein Fälligkeitsdatum"
    );
    valid = false;
  } else {
    ...
    if (!dateRegex.test(deadline)) {
      showError(
        todoDeadlineInput,
        deadlineError,
        "Bitte gib ein gültiges Datum im Format YYYY-MM-DD ein"
      );
      valid = false;
    }
  }
  ...
  if (file) {
    ...
    if (file.type !== "text/plain" && !file.name.endsWith(".txt")) {
      ...
      showError(
        todoFileInput,
        fileError,
        "Die Datei darf nicht größer als 1MB sein"
      );
      valid = false;
    }
  }
...
function saveTodosToLocalStorage() {
  try {
    ...
    if (error instanceof DOMException && error.name === "QuotaExceededError") {
      alert(
        "Der LocalStorage ist voll. Bitte lösche einige Todos, um Platz zu schaffen."
      );
    }
  }
}
...
function renderTodoList() {
  ...
  if (todos.length === 0) {
    todoList.innerHTML =
      '<li class="empty-message">Keine Aufgaben vorhanden. Erstelle deine erste Aufgabe!</li>';
    return;
  }
...
function createTodoElement(todo) {
  ...
  todoItem.innerHTML = `
    ...
    <button class="complete-btn">${
      todo.completed ? "Wiederherstellen" : "Erledigt"
    }</button>
...
function toggleTodoComplete(todoId) {
  ...
  if (todoIndex !== -1) {
    ...
    const status = todos[todoIndex].completed
      ? "erledigt"
      : "wiederhergestellt";
